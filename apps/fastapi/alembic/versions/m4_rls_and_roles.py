...
def upgrade() -> None:
    ...
    # the JWT subject via a cached subselect.
    ...
    op.execute(
        ...
        REVOKE ALL ON FUNCTION search_profiles(VECTOR, INT, UUID) FROM PUBLIC;
        GRANT EXECUTE ON FUNCTION search_profiles(VECTOR, INT, UUID) TO matching_read;
        """
    )
...
def downgrade() -> None:
    ...
    for table in ("profiles", "ideas"):
        op.execute(
            f"""
            DROP POLICY IF EXISTS {table}_own_select ON {table};
            DROP POLICY IF EXISTS {table}_own_insert ON {table};
            DROP POLICY IF EXISTS {table}_own_update ON {table};
            DROP POLICY IF EXISTS {table}_own_delete ON {table};
